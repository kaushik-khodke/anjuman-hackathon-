...
export function Signup() {
  ...
  const initialRole: "patient" | "doctor" | "pharmacist" | "hospital" =
    roleParam === "doctor" ? "doctor" :
      roleParam === "pharmacist" ? "pharmacist" :
        roleParam === "hospital" ? "hospital" : "patient";

  const [role, setRole] = useState<"patient" | "doctor" | "pharmacist" | "hospital">(initialRole);
  ...
  const onSubmit = async (data: SignupFormData) => {
    ...
    try {
      ...
      // Explicitly write role to auth user_metadata via the auth API.
      // This bypasses table-level RLS and ensures the role is always readable
      // in useAuth even if a DB trigger created the profiles row with a default role.
      const { error: metaError } = await supabase.auth.updateUser({
        data: { role },
      });
      if (metaError) {
        console.warn("Could not sync role to user metadata:", metaError.message);
      }

      // If doctor or patient, insert specific record types
      if (role === "doctor") {
        ...
        const { error: patError } = await supabase.from("patients").upsert({
          id: authData.user.id,
          user_id: authData.user.id,
          full_name: data.fullName
        }, { onConflict: 'id' });
        if (patError) {
          throw new Error("Failed to create patient profile: " + patError.message);
        }
      }
