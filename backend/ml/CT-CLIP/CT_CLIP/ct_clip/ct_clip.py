...
class CTCLIP(nn.Module):
    ...
    def load(self, path, map_location=None, strict=False):
        path = Path(path)
        assert path.exists(), f"Model path {path} does not exist!"
        if map_location is None:
            map_location = 'cuda' if torch.cuda.is_available() else 'cpu'
        pt = torch.load(str(path), map_location=map_location)
        self.load_state_dict(pt, strict=strict)

    def tokenize(self, prompt, device=None):
        if device is None:
            device = next(self.parameters()).device
        text_tokens = self.tokenizer(prompt, return_tensors="pt", padding="max_length", truncation=True, max_length=512).to(device)
        return text_tokens
